...
"""
Job 3: FedAvg with memory-efficient aggregation (memory_efficient=True) + Tensor Streaming

Memory optimization:
1. In-place aggregation (memory_efficient=True)
2. Tensor streaming (chunked transfer, no full model in memory)

Expected memory: ~4 GB -> ~1 GB (freed as aggregated + streamed)
...
# Create job
job = FedJob(name="job3_fedavg_memory_efficient")
...
# Add FedAvg workflow with MEMORY-EFFICIENT aggregation (memory_efficient=True)
workflow = FedAvg(num_clients=1, num_rounds=3, memory_efficient=True)  # MEMORY-EFFICIENT MODE - less memory!
...
if __name__ == "__main__":
    job.simulator_run(workspace="/tmp/nvflare/job3_fedavg_memory_efficient", n_clients=1)
