...
`DockerJobLauncher.__init__` reads `NVFL_DOCKER_WORKSPACE` if `workspace` is not explicitly set in `resources.json`.
...
2. `DockerJobLauncher` reads `job_image` from `meta.json`
...
Job submitted (via admin CLI or API)
  → meta.json specifies job_image per site
...
In process mode `PARENT_URL = localhost:port`. In Docker mode it must be the SP/CP **container name** on the Docker network (e.g. `server-parent:8002`) so SJ/CJ can connect back via Docker DNS.
