...
template<unsigned int TPI, unsigned int BITS>
__global__ void kernel_add(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr1, cgbn_mem_t<BITS> * arr2, int count);

template<unsigned int TPI, unsigned int BITS>
__global__ void reduce_sum(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr, int count);

template<unsigned int TPI, unsigned int BITS>
__global__ void reduce_sum_with_index(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr,
int* sample_bin, int count);
...
class PaillierCipher{
    ...
    public:
        ...
        template<unsigned int TPI, unsigned int TPB>
            int add(cgbn_mem_t<BITS>* d_res_ptr, cgbn_mem_t<BITS>* d_arr1_ptr, cgbn_mem_t<BITS>* d_arr2_ptr,int count) {
                int IPB=TPB/TPI;

                cgbn_error_report_t *report;
                ck(cgbn_error_report_alloc(&report));
                kernel_add<TPI, BITS><<<(count+IPB-1)/IPB, TPB>>>(report, d_res_ptr, d_arr1_ptr, d_arr2_ptr, count);
#ifdef DEBUG
                ck(cudaDeviceSynchronize());
                ck(cudaGetLastError());
                CGBN_CHECK(report);
        ...
        template<unsigned int TPI, unsigned int TPB>
            int sum(cgbn_mem_t<BITS>* d_res_ptr, cgbn_mem_t<BITS>* d_arr_ptr, int count) {
                int IPB=TPB/TPI;
                int maxBlocks = 2560;
                int numBlocks = min((count - 1) / IPB + 1, maxBlocks);
                int mem_size = numBlocks * sizeof(cgbn_mem_t<BITS>);
                if (count == 0) {
                    cudaMemcpy(d_res_ptr, &pub_key.rand_seed, sizeof(cgbn_mem_t<BITS>), cudaMemcpyHostToDevice);
                    return 0;
                }
                ...
                cgbn_mem_t<BITS>* d_res_ptr_2;
                ck(cudaMalloc((void **)&d_res_ptr_2, mem_size));
 
                typedef cgbn_context_t<TPI>         context_t;
                typedef cgbn_env_t<context_t, BITS> env_t;
                typedef typename env_t::cgbn_t bn_t;
                int shmem_size = IPB * sizeof(cgbn_mem_t<BITS>);

#ifdef DEBUG
                std::cout << "before calling reduce_sum with cgbn_mem_t" << std::endl;
                ...
                reduce_sum<TPI, BITS><<<numBlocks, TPB, shmem_size>>>(report, d_res_ptr_2, d_arr_ptr, count);
                ck(cudaDeviceSynchronize());
#ifdef DEBUG                 
                std::cout << "after calling reduce_sum with cgbn_mem_t" << std::endl;
#endif

                // final reduction
                if (numBlocks != 1) {
                    reduce_sum<TPI, BITS><<<1, TPB, shmem_size>>>(report, d_res_ptr, d_res_ptr_2, numBlocks);
                } else {
...
__global__ void kernel_add(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr1, cgbn_mem_t<BITS> * arr2, int count) {
    ...
    cgbn_load(bn_env, a, arr1 + tid);
    cgbn_load(bn_env, b, arr2 + tid);
    cgbn_mul_wide(bn_env, r, a, b);
    cgbn_load(bn_env, b, &c_PubKey.n_square);
    cgbn_rem_wide(bn_env, a, r, b);
    cgbn_store(bn_env, result + tid, a);
}
...
__global__ void reduce_sum(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr, int count) {
    ...
    extern __shared__ cgbn_mem_t<BITS> sdata1[];
    bn_t a, b, c, tmp;
    bn_t n_square;
    bn_w_t r;
    ...
    for (unsigned int window = 0; window < total_windows; window++) {
...
#ifdef DEBUG
        printf("id %d shm_id %d IPB %d gridDim.x %d window %d total_windows %d global_position %d \n", id, shm_id, IPB, gridDim.x, window, total_windows, global_position);
#endif

        if (global_position >= count) {
            // if global position is greater than count, load rand_seed, it is the same as an encrypted 0
            sdata1[shm_id] = c_PubKey.rand_seed;
        } else {
            // each shm_id copy one instance from global to shared mem
            sdata1[shm_id] = arr[global_position];
        }
        __syncthreads();
        ...
        for (unsigned int s = IPB / 2; s > 0 ; s >>= 1) {
            if (shm_id < s) {
                cgbn_load(bn_env, a, &(sdata1[shm_id]));
                cgbn_load(bn_env, b, &(sdata1[shm_id + s]));
                cgbn_mul_wide(bn_env, r, a, b);
                cgbn_rem_wide(bn_env, c, r, n_square);
		        cgbn_store(bn_env, &(sdata1[shm_id]), c);
	        }
            __syncthreads();
        }
        ...
        if (shm_id == 0) {
            if (window == 0) {
                cgbn_load(bn_env, tmp, &(sdata1[0]));
            } else {
                cgbn_load(bn_env, a, &(sdata1[0]));
                cgbn_mul_wide(bn_env, r, a, tmp);
                cgbn_rem_wide(bn_env, tmp, r, n_square);
            }
        }
    }
    ...
    if (shm_id == 0) {
        cgbn_store(bn_env, result + blockIdx.x, tmp);
    }
}
...
__global__ void reduce_sum_with_index(cgbn_error_report_t *report, cgbn_mem_t<BITS>* result, cgbn_mem_t<BITS> * arr,
    ...
    context_t bn_context(cgbn_report_monitor, report, id);
    env_t bn_env(bn_context);
    extern __shared__ cgbn_mem_t<BITS> sdata2[];
    bn_t a, b, c, tmp;
    bn_t n_square;
    bn_w_t r;
    ...
    for (unsigned int window = 0; window < total_windows; window++) {
        ...
        if (global_position >= count) {
            // if global position is greater than count, load rand_seed, it is the same as an encrypted 0
            sdata2[shm_id] = c_PubKey.rand_seed;
        } else {
            // each shm_id copy one instance from global to shared mem
            int sample_id = sample_bin[global_position];
            sdata2[shm_id] = arr[sample_id];
        }
        __syncthreads();
        ...
        for (unsigned int s = IPB / 2; s > 0 ; s >>= 1) {
            if (shm_id < s) {
                cgbn_load(bn_env, a, &(sdata2[shm_id]));
                cgbn_load(bn_env, b, &(sdata2[shm_id + s]));
                cgbn_mul_wide(bn_env, r, a, b);
                cgbn_rem_wide(bn_env, c, r, n_square);
		        cgbn_store(bn_env, &(sdata2[shm_id]), c);
	        }
            __syncthreads();
        }
        ...
        if (shm_id == 0) {
            if (window == 0) {
                cgbn_load(bn_env, tmp, &(sdata2[0]));
            } else {
                cgbn_load(bn_env, a, &(sdata2[0]));
                cgbn_mul_wide(bn_env, r, a, tmp);
                cgbn_rem_wide(bn_env, tmp, r, n_square);
            }
        }
