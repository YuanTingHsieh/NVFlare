...
try:
    import phe
    from phe.paillier import EncryptedNumber as EncryptedNumber
    USE_PHE = True
except:
    import ipcl_python
...
def generate_keys(n_length=1024):
    if USE_PHE:
        return phe.paillier.generate_paillier_keypair(n_length=n_length)
    else:
...
def encrypt_number(pubkey, ciphertext, exponent):
    if USE_PHE:
        return EncryptedNumber(pubkey, ciphertext, exponent)
    else:
...
def create_pub_key(key, n_length=1024):
    if USE_PHE:
        return phe.paillier.PaillierPublicKey(n=key)
    else:
...
def int_to_ciphertext(d, pubkey = None):
    if USE_PHE:
        return d
    else:
        return ipcl_python.ipclCipherText(
            pubkey.pubkey, ipcl_python.BNUtils.int2BN(d)
        )
...
def _decode_encrypted_numbers(pubkey, data):
    ...
    for v in data:
        if isinstance(v, int):
            ...
            d = encrypt_number(pubkey, ciphertext=int_to_ciphertext(base64_to_int(v[0]), pubkey=pubkey), exponent=int(v[1]))
        result.append(d)
    return result
