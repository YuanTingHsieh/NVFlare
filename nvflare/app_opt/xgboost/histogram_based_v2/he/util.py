...
def combine(g, h):
    return np.array([g, h]).astype(int)
...
def _encode_encrypted_numbers(numbers):
    ...
    for x in numbers:
        if isinstance(x, EncryptedNumber):
            result.append(
                (
                    [int_to_base64(ipcl_python.BNUtils.BN2int(i)) for i in x.ciphertextBN()],
                    [int_to_base64(j) for j in x.exponent()]
                )
            )
        else:
            result.append(x)
...
def _decode_encrypted_numbers(pubkey, data):
    ...
    for v in data:
        if isinstance(v, int):
            ...
            d = EncryptedNumber(
                pubkey,
                ciphertext=ipcl_python.ipclCipherText(pubkey.pubkey, [ipcl_python.BNUtils.int2BN(int(base64_to_int(i))) for i in v[0]]),
                exponents=[(int(base64_to_int(j))) for j in v[1]],
                length=len(v[0])
                )
        result.append(d)
    return result
